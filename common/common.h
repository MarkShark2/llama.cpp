...
struct common_params_speculative_draft {
    ...
    int32_t n_ubatch = 0; // physical batch size of the draft context (0 = inherit the target's)
...
struct common_params {
    ...
    // like fit_params_fill_rpc_first, but overflow the primary device cannot hold
    // falls fully to the CPU instead of aborting (RPC-safe). Kept at the end of the
    // struct so adding it never shifts the offset of any pre-existing field.
    bool fit_params_cpu_moe = false;
};
