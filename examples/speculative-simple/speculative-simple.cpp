...
int main(int argc, char ** argv) {
    ...
    common_state_seq_io_ctx io_tgt(ctx_tgt);
    common_state_seq_io_ctx io_dft(ctx_dft.get());
    ...
    while (true) {
        ...
        if (draft.empty()) {
            ...
            if (use_ckpt_dft) {
                ckpt.update_dft(io_dft, seq_id, LLAMA_STATE_SEQ_FLAGS_PARTIAL_ONLY);
            }
            ...
            if (!draft.empty()) {
                if (use_ckpt_tgt) {
                    ckpt.update_tgt(io_tgt, seq_id, LLAMA_STATE_SEQ_FLAGS_PARTIAL_ONLY);
                }
            }

            {
                ckpt.load_dft(io_dft, seq_id, LLAMA_STATE_SEQ_FLAGS_PARTIAL_ONLY);
        ...
        if (use_ckpt_tgt && ids.size() - 1 < draft.size()) {
            ...
            {
                ckpt.load_tgt(io_tgt, seq_id, LLAMA_STATE_SEQ_FLAGS_PARTIAL_ONLY);
