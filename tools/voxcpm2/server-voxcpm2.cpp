...
// Ported from tc-mb/llama.cpp-omni tools/server/server-voxcpm2.cpp.
// Fork deltas: the voxcpm2 model args are parsed locally instead of patching
// common/ (upstream omni added them to common_params), --model is accepted as
// an alias for --voxcpm2-base-lm, and the listener only uses SSLServer when a
// certificate is configured (matching llama-server).
...
int main(int argc, char ** argv) {
    ...
    // voxcpm2-specific args are stripped before common_params_parse sees argv
    std::string arg_base_lm;
    std::string arg_acoustic;
    int         arg_n_gpu_layers = -1;
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "--voxcpm2-base-lm") {
            take_value(arg_base_lm);
        } else if (arg == "--voxcpm2-acoustic") {
            take_value(arg_acoustic);
        } else if (arg == "--voxcpm2-n-gpu-layers") {
            std::string v;
            take_value(v);
            arg_n_gpu_layers = atoi(v.c_str());
            has_n_gpu_layers = true;
        } else {
            filtered_argv.push_back(argv[i]);
        }
    }

    if (!common_params_parse((int) filtered_argv.size(), filtered_argv.data(), params, LLAMA_EXAMPLE_SERVER)) {
        return 1;
    }

    // --model doubles as the BaseLM path so standard launch configs work
    if (arg_base_lm.empty()) {
        arg_base_lm = params.model.path;
    }
    if (!has_n_gpu_layers) {
        arg_n_gpu_layers = params.n_gpu_layers;
    }
    ...
    if (!params.ssl_file_cert.empty() && !params.ssl_file_key.empty()) {
        svr_ptr = std::make_unique<httplib::SSLServer>(params.ssl_file_cert.c_str(), params.ssl_file_key.c_str());
    } else {
    ...
    bool has_voxcpm2 = !arg_base_lm.empty() && !arg_acoustic.empty();

    if (!has_voxcpm2) {
        LOG_ERR("No VoxCPM2 models specified. Provide --voxcpm2-base-lm (or --model) and --voxcpm2-acoustic\n");
        llama_backend_free();
        return 1;
    }
    ...
    {
        auto * rt = new VoxCPM2Runtime();
        if (!rt->init(arg_base_lm, arg_acoustic, arg_n_gpu_layers, /*use_gpu_backend=*/true)) {
