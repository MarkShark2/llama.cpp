...
static llama_kv_cache_dsv4_context::comp_plan dsv4_build_comp_plan(
    ...
    // [fork] with LLAMA_DSV4_PLAN_BUCKET the same treatment extends to the HCA
    ...
    if ((ratio == DSV4_CSA_RATIO || dsv4_plan_bucket_step() > 0) && !plan.state_pos.empty()) {
        assert(kv_size > 0);
...
bool llama_kv_cache_dsv4::seq_rm(llama_seq_id seq_id, llama_pos p0, llama_pos p1) {
    ...
    if (p0 > 0) {
        ...
        // Bounded partial (suffix) rollback.
        //
...
void llama_kv_cache_dsv4::state_read(llama_io_read_i & io, llama_seq_id seq_id, llama_state_seq_flags flags) {
    ...
    if (!partial_only) {
        clear_compressed(seq_id, true, DSV4_POS_ALL);
...
// written; DSV4_POS_ALL clears the whole per-stream slice. [fork]
