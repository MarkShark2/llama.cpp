...
llm_graph_context::llm_graph_context(const llm_graph_params & params) :
    ...
    pipedec_lane     (params.pipedec_lane),
    pipedec_total    (params.pipedec_total),
        ...
        case LLM_FFN_SILU:
            if (gate && type_gate == LLM_FFN_PAR) {
                if (il >= 0) {
                    ...
                    if (limit > eps) {
                        // [fork] GLM5NEXT shares DSV4's clamp semantics; upstream's fused
                        // ggml_swiglu_clamp is exactly min(gate,limit) / clamp(up,+-limit) /
                        // swiglu_split, so it replaces the three-op sequence here.
        ...
        case LLM_FFN_SILU:
            if (gate_exps) {
                if (il >= 0) {
                    ...
                    constexpr float eps = 1e-6f;
                    if (limit > eps) {
                        if (arch == LLM_ARCH_DEEPSEEK4 || arch == LLM_ARCH_GLM5NEXT || arch == LLM_ARCH_HY_V4 ||
                                (arch == LLM_ARCH_DFLASH && hparams.dsv4_hc_mult > 0)) {
