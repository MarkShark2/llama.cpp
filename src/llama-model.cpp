                ...
                if (llm_arch_is_recurrent(arch)) {
                    ...
                    if (hparams.swa_type != LLAMA_SWA_TYPE_NONE) {
                        ...
                        res = new llama_memory_hybrid_iswa(
                            ...
                            /* attn_kv_size      */ cparams.n_ctx_seq,
                            /* attn_n_ubatch     */ cparams.n_ubatch_nc,
                            /* attn_n_pad        */ 1,
                            /* recurrent_type_r  */ GGML_TYPE_F32,
                            /* recurrent_type_s  */ GGML_TYPE_F32,
                            /* recurrent_rs_size */ std::max((uint32_t) 1, cparams.n_seq_max),
                    ...
                    if (hparams.swa_type != LLAMA_SWA_TYPE_NONE && hparams.is_swa_any()) {
                        ...
                        if (hparams.swa_type != LLAMA_SWA_TYPE_NONE && !params.swa_full) {
                            ...
                            kv_size = GGML_PAD(std::min(kv_size, n_swa_seq + cparams.n_ubatch_nc), 256);
...
llama_rope_type llama_model_rope_type(const llama_model * model) {
    switch (model->arch) {
        ...
        case LLM_ARCH_DFLASH:
            ...
            // Qwen3-style backbone pairs them like Qwen (NEOX). dflash_dsv4_backbone is
            // probed from the MLA tensors, hc_mult comes from the DSpark hparams.
            return (model->hparams.dflash_dsv4_backbone || model->hparams.dsv4_hc_mult > 0)
                ? LLAMA_ROPE_TYPE_NORM : LLAMA_ROPE_TYPE_NEOX;
