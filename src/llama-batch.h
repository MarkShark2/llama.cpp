...
struct llama_ubatch {
    ...
    uint32_t n_embd;       // [fork] floats per embd row, the width the allocator strided batch.embd at
    ...
    llama_token  *  routing_id; // [n_tokens]         | i   | routing id for embd input (see llama_batch), null otherwise
...
public:
    ...
    // [fork] batch index of every token in the order the ubatches took them
    const std::vector<int32_t> & get_tok_ids() const { return tok_ids; }
...
private:
    ...
    // [fork] batch indices of all tokens, in ubatch order
