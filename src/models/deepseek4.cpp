...
ggml_tensor * llama_model_deepseek4::graph::build_lid_top_k(
    ...
    // null when the cache holds the constant device-resident (dev_k_rot)
    ggml_tensor * lid_k_rot = inp_dsv4->get_lid_k_rot(il);

    GGML_ASSERT(inp_lid.kq_mask);
    GGML_ASSERT(lid_k_rot);
...
ggml_tensor * llama_model_deepseek4::graph::build_csa_lid_attention(
    ...
    if (llama_dsv4_sparse_attn() && cparams.flash_attn && inp_csa.n_vis) {
        ...
        out = build_attn_mha(q, k_all, k_all, nullptr, kq_mask, sinks, nullptr, n_kv_max, kq_scale, il);
    }
...
ggml_tensor * llama_model_deepseek4::graph::build_attention_impl(
    ...
    if (ratio == DSV4_CSA_RATIO && inp_dsv4->get_csa().state_pos) {
        ...
        if (inp_dsv4->get_csa_k_rot(il)) {
            kv_comp_csa_state = llama_mul_mat_hadamard(ctx0, kv_comp_csa_state, inp_dsv4->get_csa_k_rot(il));
        ...
        if (inp_dsv4->get_lid_k_rot(il)) {
            kv_comp_lid_state = llama_mul_mat_hadamard(ctx0, kv_comp_lid_state, inp_dsv4->get_lid_k_rot(il));
    ...
    if (ratio == DSV4_HCA_RATIO && inp_dsv4->get_hca().state_write_idxs) {
        ...
        if (inp_dsv4->get_hca_k_rot(il)) {
            kv_comp_hca = llama_mul_mat_hadamard(ctx0, kv_comp_hca, inp_dsv4->get_hca_k_rot(il));
...
llama_model_deepseek4::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    for (int il = layer_start; il < layer_end; ++il) {
        ...
        // may apply exp_probs_b_vl is input is from mtmd; [fork] an embedding batch that
        // carries routing ids (the DSpark/MoonViT palette) keeps the hash-layer gather instead
        const bool is_media = ubatch.embd != nullptr && ubatch.routing_id == nullptr;
        if (is_media) {
    ...
    // The nextn tap is the wide pre-collapse residual: that is what graph_mtp
    // (n_embd_out == n_embd*hc) and the DSpark drafter's feature extraction read.
    ...
    // [fork] Stage 2 PipeDec queues token-sized trunk graphs across the layer
    // devices. The body ends at the collapsed pre-final-norm hidden state; those
    // rows are copied out and fed to one batched graph_pipedec_head (output norm
    // + lm_head) after the body pipeline drains. The HC collapse stays in the body
    // so the carried state is [n_embd, n_tokens] like every other arch -- which is
    // why the body overrides the wide nextn tap set above.
