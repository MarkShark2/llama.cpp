...
void llama_model_qwen4exp::load_arch_tensors(llama_model_loader & ml) {
    ...
    // [fork] the draft head ships as its own GGUF, so it can be quantized separately from the
    // trunk and pinned to the draft device (--model-draft + --device-draft) instead of riding
    // the trunk's tensor-split out onto the RPC fabric. Both halves declare nextn in their
    // metadata, so probe for the tensors: a draft-only file has no blk.0, a trunk-only file
    // has no nextn block. mtp_use_dedicated_embeddings is false for qwen4exp, so the draft
    // file still carries token_embd and output -- those stay required in both halves.
    ...
    // there is no output_norm: the final hyper-connection mixer carries it. The draft head has
    // its own copy under nextn.hc_head_*, so these are trunk-only.
    ...
    // MTP tensors sit in the trailing blocks: absent from a trunk-only file, and skipped
    // entirely unless a draft head was asked for
    ...
    for (int il = 0; il < (int) hparams.n_layer_all; ++il) {
        ...
        // [fork] n_ff_exp is a per-layer accessor here; the raw field is n_ff_exp_impl
