...
void llama_model_dflash::load_arch_hparams(llama_model_loader & ml) {
    ...
    // [fork] Laguna drafters follow the target-architecture decoder contract; see the
    ...
    if (ml.get_key(LLM_KV_ATTENTION_SLIDING_WINDOW, hparams.n_swa, false) && hparams.n_swa > 0) {
        ...
        if (!ml.get_key_or_arr(LLM_KV_ATTENTION_SLIDING_WINDOW_PATTERN, hparams.is_swa_impl, hparams.n_layer(), false)) {
            hparams.set_swa_pattern(0); // [fork] no pattern key: every layer is SWA
        }
        hparams.rope_freq_base_train_swa  = hparams.rope_freq_base_train;
        hparams.rope_freq_scale_train_swa = hparams.rope_freq_scale_train;
    }
...
void llama_model_dflash::load_arch_tensors(llama_model_loader &) {
    ...
    // [fork] Laguna drafters norm each captured target feature before concat + fc;
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        // [fork] Optional attention output gate (Laguna drafters). Per-head or
...
llama_model_dflash::graph<true>::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    // [fork] Laguna drafters RMS-norm each captured feature before concat + fc.
...
llama_model_dflash::graph<false>::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    if (ubatch.embd) {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // [fork] Laguna draft layers project context K/V from the input_layernorm
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // [fork] with a Laguna gate present, o_proj is deferred until after gating
