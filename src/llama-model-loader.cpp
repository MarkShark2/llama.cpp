...
struct rpc_manifest {
    ...
    int64_t mtime = 0;
    std::unordered_map<std::string, rpc_manifest_entry> entries;
    bool dirty = false;
};
...
static uint64_t rpc_tensor_hash(const uint8_t * data, size_t size) {
...
static int64_t rpc_file_mtime(const std::string & path, std::error_code & ec) {
    const auto value = fs::last_write_time(path, ec);
    if (ec) {
        return 0;
    }
    return (int64_t) value.time_since_epoch().count();
}

static rpc_manifest rpc_manifest_load(const std::string & model_path) {
    rpc_manifest result;
    if (model_path.empty()) {
        return result;
    }
    ...
    std::error_code ec;
    result.path = model_path + ".rpc-cache-manifest";
    result.file_size = fs::file_size(model_path, ec);
    if (ec) {
        return result;
    }
    result.mtime = rpc_file_mtime(model_path, ec);
    if (ec) {
        return result;
    }
    ...
    uintmax_t file_size = 0;
    int64_t mtime = 0;
    if (!(input >> magic >> file_size >> mtime) ||
            magic != "llama-rpc-cache-manifest-v1" ||
            file_size != result.file_size || mtime != result.mtime) {
        return result;
    }
    ...
    while (std::getline(input, line)) {
        ...
        std::string name;
        if (row >> std::hex >> entry.hash >> std::dec >> entry.offset >> entry.size >> name) {
            result.entries.emplace(std::move(name), entry);
        }
    }
    return result;
}

static bool rpc_manifest_save(const rpc_manifest & manifest) {
    if (!manifest.dirty || manifest.path.empty()) {
        return true;
    }

    const std::string temp_path = manifest.path + ".tmp";
    std::ofstream output(temp_path, std::ios::trunc);
    if (!output) {
        return false;
    }
    output << "llama-rpc-cache-manifest-v1 " << manifest.file_size << ' ' << manifest.mtime << '\n';
    for (const auto & item : manifest.entries) {
        output << std::hex << std::setw(16) << std::setfill('0') << item.second.hash
               << std::dec << ' ' << item.second.offset << ' ' << item.second.size << ' ' << item.first << '\n';
    }
    output.close();
    if (!output) {
        return false;
    ...
    std::error_code ec;
    fs::rename(temp_path, manifest.path, ec);
    if (!ec) {
        return true;
    }
    fs::remove(manifest.path, ec);
    ec.clear();
    fs::rename(temp_path, manifest.path, ec);
    return !ec;
}
...
bool llama_model_loader::load_all_data_parallel(
    ...
    if (files.empty() || use_direct_io || file_paths.size() != files.size()) {
