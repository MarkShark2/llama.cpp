...
class DeepseekV4DSparkModel(DeepseekV4Model):
    ...
    def index_tensors(self, remote_hf_model_id: str | None = None) -> dict[str, Callable[[], Tensor]]:
        ...
        # a local drafter checkpoint ships only its own shards (3 of the target's 48), but
        # model.safetensors.index.json still lists the full target weight map -- read the shards
        # actually present on disk instead of trusting it (filter_tensors then drops anything
        # that is not a "mtp.*" tensor)
    ...
    def filter_tensors(cls, item: tuple[str, Callable[[], Tensor]]) -> tuple[str, Callable[[], Tensor]] | None:
        ...
        # target backbone tensors, silently drop anything that is not a draft tensor
